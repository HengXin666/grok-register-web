...
**只需要一份 compose YAML，无需源码、无需 build：**

```bash
# 下载 compose 后直接启动（会 pull GHCR 镜像）
curl -fsSL -o docker-compose.yml \
  https://raw.githubusercontent.com/HengXin666/grok-register-web/master/docker-compose.yml
...
数据卷：`grok-register-data`。  
推荐：**协议注册 + 外置 Turnstile Solver**（网页设置里配 solver URL）；容器内不要依赖 headful Chrome。

开发本地构建才需要：`docker compose -f docker-compose.yml -f docker-compose.build.yml up -d --build`。
